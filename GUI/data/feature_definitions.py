...
Each feature category is defined in a tuple with the following format:
(category_name, features_data, grid_row, grid_column)

Features can be defined in two formats:
1. Simple string list: Just the feature names as they appear in the data
2. Dictionary list: More complex format for features that need additional display information
   - For combined left/right metrics: {"name": "Display Name", "left": "left_eye_column", "right": "right_eye_column"}
   - For single metrics: {"name": "Display Name", "key": "data_column_name"}

Grid rows and columns define the layout position in the feature display.
"""
